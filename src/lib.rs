//! komga service backend — Comics/manga server.
//!
//! Implements `ServiceBackend` so the generic `service.*` tools
//! (deploy/backup/restore/configure/status/connect/sync) drive komga. No
...
/// komga backend. Holds only the provider name; per-instance endpoint/creds
/// come from the `Endpoint` the generic `service.*` tools hand each op.
#[derive(Debug, Clone)]
pub struct KomgaBackend {
    provider: &'static str,
}
...
impl ServiceBackend for KomgaBackend {
    fn provider(&self) -> &str { self.provider }

    /// Runtimes komga can be placed on. `service.deploy` hands the
        ...
        -> BoxFuture<'a, Result<WorkloadSpec, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("komga.workload_spec")) })
    }
        ...
        -> BoxFuture<'a, Result<BackupArtifact, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("komga.backup")) })
    }

    fn restore<'a>(&'a self, _ep: &'a Endpoint, _from: &'a BackupArtifact)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        Box::pin(async move { Err(ServiceError::unimplemented("komga.restore")) })
    }

    fn configure<'a>(&'a self, _ep: &'a Endpoint, _config: &'a str)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        // TODO: apply komga-specific config idempotently.
        Box::pin(async move { Err(ServiceError::unimplemented("komga.configure")) })
    }
        ...
        -> BoxFuture<'a, Result<ServiceStatus, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("komga.status")) })
    }
}
...
mod tests {
    ...
    fn declares_provider() {
        let b = KomgaBackend::new("komga");
        assert_eq!(b.provider(), "komga");
    }
}
